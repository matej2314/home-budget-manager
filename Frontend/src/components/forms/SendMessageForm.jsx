...
export default function SendMessageForm({ reply, recipientName, onClose }) {
	...
	return (
		<div className="w-8/12 xl:w-10/12 h-full flex flex-col justify-center items-center gap-4">
			<h2 className="text-xl xl:text-2xl mb-2">{reply ? 'Reply message' : 'Send message'}</h2>
			<form onSubmit={handleSendMessage} className="w-full h-full flex flex-col items-center justify-center gap-3">
				...
				<div className="relative w-fit flex justify-center">
					<input type="text" name="recipientName" id="recipientName" placeholder="recipient name" ref={recipientRef} defaultValue={recipientName || ''} required disabled={recipientName} onInput={(e) => (e.target.nextSibling.style.display = e.target.value ? 'none' : 'block')} className={`${recipientName ? 'bg-gray-300 cursor-not-allowed' : ''} input-base`} />
				...
				<div className="relative w-[13rem] indirect:w-full indirectxl:w-10/12  sm:w-9/12 md:w-8/12 lg:w-6/12 xl:w-11/12 flex justify-center">
					<textarea className=" resize-none input-base" name="messageContent" id="messageContent" placeholder="message" ref={messageContentRef} required />
