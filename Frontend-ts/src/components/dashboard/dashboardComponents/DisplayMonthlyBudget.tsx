...
export default function MonthlyBudget() {
    ...
    const [currentInitBudget, setCurrentInitBudget] = useState<number>();
    ...
    useEffect(() => {
        if (!isLoading && !error && data?.houseData?.length > 0) {
            ...
            setCurrentInitBudget(Number(lastInitBudget));
    ...
    useEffect(() => {
        if (!socketError && connected && messages?.initBudgets?.length > 0) {
            ...
            setCurrentInitBudget(() => latestBudget?.data.initBudget);
            setCurrentBudgetPeriod(() => latestBudget?.data.budgetPeriod);
        }
