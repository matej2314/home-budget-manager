...
export default function LossOrGainChart() {
    ...
    const statData = getData(isLoading, String(error), true, statsData, []);
    ...
    const generateBarColors = (values: number[]) => {
    ...
    const hasLoss = monthlyBalances.some(value => value < 0);
    const chartTitle = hasLoss ? t("lossOrGain.loss") : t("lossOrGain.gain");
