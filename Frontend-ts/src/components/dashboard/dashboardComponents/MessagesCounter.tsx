...
export default function MessagesCounter() {
    ...
    const socketNewMessages = messages && messages.newMessages.length > 0 ? messages.newMessages : [];

    useEffect(() => {
        if (connected && !error && socketNewMessages) {
            const userMessages = filterArray(socketNewMessages, (message) => message.type === 'newMessage');
    ...
    return (
        ...
        >
            <div className="w-full flex flex-col items-center justify-start -translate-y-1 translate-x-1 gap-5">
                ...
                <span className="text-xl">{(userMessages as NewMessageType[]).length > 0 ? userMessages!.length : 0}</span>
            </div>
