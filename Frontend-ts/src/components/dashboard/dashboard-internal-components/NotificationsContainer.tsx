...
import { type  NotificationCategory, NoticeData } from "@models/notificationsStoreTypes";


interface NotificationsContainerInput {
    notifications: unknown;
    clickAction: (type: NotificationCategory, id: string) => void;
    onClose: () => void;
}

export default function NotificationsContainer({ notifications, clickAction }: NotificationsContainerInput) {
    const { data, isLoading } = useContext(DataContext);
    const { t: tInternal } = useTranslation("dashboardInternal");
    const { t: tCommon } = useTranslation("common")

    const houseName = !isLoading && !data.dataError && data.houseData[0].houseName || '';
            ...
            {notifications ? (
                Object.entries(notifications).some(([_, items]) => items.length > 0) ? (
                    <ul className='w-full flex flex-col gap-2'>
                        {(Object.entries(notifications) as [NotificationCategory, NoticeData[]][]).map(([category, items]) => items.map((notification: NoticeData, index) => (
                            <li
                                key={`${category}-${index}`}
