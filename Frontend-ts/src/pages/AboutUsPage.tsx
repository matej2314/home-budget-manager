...
import TechnologiesSection from '@components/homePageComponents/TechnologiesSection';
import DescriptionSection from '@components/homePageComponents/DescriptionSection';
import LoadingModal from '../components/modals/LoadingModal';
import { type Shots, Reviews } from '@models/homePageStoreTypes';
...
export default function AboutUs() {
    ...
    const technologiesData = !isHomePageDataLoading && !homePageDataError ? homePageData?.technologies : [];
    const photosData = !isHomePageDataLoading && !homePageDataError ? homePageData?.shots : [];
    const reviewsData = !isHomePageDataLoading && !homePageDataError ? homePageData?.reviews : [];
    ...
    return (
        <div className="w-full h-full flex flex-col justify-start xl:px-16 overflow-auto">
            <div className="w-full h-[90%] flex flex-col justify-start items-center md:gap-4 rounded-md overflow-auto no-scrollbar pb-[4rem] mt-[4rem] indirect:mt-[3rem] indirectxl:mt-[3rem] sm:mt-[3rem] md:mt-5 lg:mt-[3rem] xl:mt-0">
                <DescriptionSection />
                <TechnologiesSection technologies={technologiesData} isMobile={isMobile} />
                <div className="w-full h-fit flex flex-col justify-center items-start text-slate-100 gap-3">
                    ...
                    <div className="w-full h-[18rem] xl:h-[16rem] flex justify-center items-start">
                        <div className="w-full max-w-full h-full flex justify-center items-center rounded-md">
                            <AppGallery photos={photosData as Shots} />
                        </div>
                    </div>
                </div>
                <ReviewsSection reviews={reviewsData as Reviews} />
            </div>
        </div>
    )
}